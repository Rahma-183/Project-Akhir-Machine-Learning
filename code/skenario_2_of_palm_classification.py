# -*- coding: utf-8 -*-
"""Skenario 2 of palm_classification.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1qHuL9EuxLPaE30UjUhYbIwSgWcQiN1An
...
"""### **7.1 PHASE 1 Feature Extraction (backbone FROZEN)**"""
...
"""### 7.2 PHASE 2 Fine-Tuning (ALL layers UNFROZEN)"""
...
cnn_micro_f1 = f1_score(all_labels, all_preds, average='micro',    zero_division=0)
cnn_kappa    = cohen_kappa_score(all_labels, all_preds)

print('=' * 55)
print('HASIL EVALUASI PADA TEST SET')
print('=' * 55)
print(f'  Accuracy       : {cnn_acc:.4f}  ({cnn_acc*100:.2f}%)')
print(f'  Macro F1-Score : {cnn_macro_f1:.4f}')
print(f'  Micro F1-Score : {cnn_micro_f1:.4f}')
...
"""##**10. Baseline: Machine Learning Klasik**

### **10.1.Install & Import tambahan**
"""
...
def extract_features(image_path, img_size=128):
    ...
    # HOG: menangkap bentuk dan tepi
    hog_feat = hog(gray, orientations=8, pixels_per_cell=(8, 8),
                   cells_per_block=(2, 2), feature_vector=True)

    # Color Histogram RGB
    hist_r = np.histogram(arr[:,:,0], bins=32, range=(0,256))[0].astype(float)
    hist_g = np.histogram(arr[:,:,1], bins=32, range=(0,256))[0].astype(float)
    hist_b = np.histogram(arr[:,:,2], bins=32, range=(0,256))[0].astype(float)
    hist_r /= (hist_r.sum() + 1e-7)
    hist_g /= (hist_g.sum() + 1e-7)
    hist_b /= (hist_b.sum() + 1e-7)

    # Color Histogram HSV
    arr_hsv = (color.rgb2hsv(arr) * 255).astype(np.uint8)  # scale ke 0-255
    hist_h = np.histogram(arr_hsv[:,:,0], bins=32, range=(0,256))[0].astype(float)
    hist_s = np.histogram(arr_hsv[:,:,1], bins=32, range=(0,256))[0].astype(float)
    hist_h /= (hist_h.sum() + 1e-7)
    hist_s /= (hist_s.sum() + 1e-7)

    color_feat = np.concatenate([hist_r, hist_g, hist_b, hist_h, hist_s])

    # LBP: menangkap tekstur permukaan
    lbp = local_binary_pattern(gray, P=24, R=3, method='uniform')
    lbp_hist = np.histogram(lbp, bins=26, range=(0, 26))[0].astype(float)
    lbp_hist /= (lbp_hist.sum() + 1e-7)

    return np.concatenate([hog_feat, color_feat, lbp_hist])

"""###**10.3 Ekstrak fitur dari semua split**"""

def build_feature_matrix(split_dir, class_names):
    X, y = [], []
    for label, cls in enumerate(class_names):
        paths = list((split_dir / cls).glob('*.[jp][pn]g')) + \
                list((split_dir / cls).glob('*.jpeg'))
        for p in paths:
...
X_train, y_train = build_feature_matrix(TRAIN_DIR, CLASS_NAMES)
print('Mengekstrak fitur val...')
X_val, y_val = build_feature_matrix(VAL_DIR, CLASS_NAMES)
print('Mengekstrak fitur test...')
X_test, y_test = build_feature_matrix(TEST_DIR, CLASS_NAMES)

print(f'Shape X_train: {X_train.shape}')

"""###**10.4 Training model ML**"""
...
search = RandomizedSearchCV(
    ...
    n_iter=20,              #  20 kombinasi acak
    cv=ps,                  # pakai val set dari Roboflow
    scoring='f1_macro',
    random_state=SEED,
    n_jobs=-1,
    verbose=1
...
print(f'Best val Macro F1 : {search.best_score_:.4f}')

svm_pipeline = search.best_estimator_
print('\nSVM pipeline diperbarui dengan best estimator ✓')

"""###**10.5 Evaluasi**"""

results = {}
for name, model_ml in [('SVM', svm_pipeline)]:
    results[name] = {}
    for split_name, X_s, y_s in [('Val', X_val, y_val), ('Test', X_test, y_test)]:
        y_pred = model_ml.predict(X_s)
        acc   = accuracy_score(y_s, y_pred)
        f1    = f1_score(y_s, y_pred, average='macro', zero_division=0)
        kappa = cohen_kappa_score(y_s, y_pred)

        results[name][split_name] = {'acc': acc, 'f1': f1, 'kappa': kappa}

        print(f'\n{name} — {split_name} Set')
        print(f'  Accuracy    : {acc:.4f}')
        print(f'  Macro F1    : {f1:.4f}')
        print(f'  Cohen Kappa : {kappa:.4f}')
        print(classification_report(y_s, y_pred, target_names=CLASS_NAMES, zero_division=0))
...
for ax, data, fmt, title in zip(
    axes,
    [cm_svm, cm_svm.astype(float) / cm_svm.sum(axis=1, keepdims=True)],
    ['d', '.2f'],
...
summary_cnn = pd.DataFrame({
    'Kelas'    : CLASS_NAMES,
    'Precision': precision.round(4),   # dari Cell 39 (CNN)
...
kappa_interp = (
    'Hampir sempurna (>0.80)' if cnn_kappa > 0.80 else
    'Substansial (0.61-0.80)' if cnn_kappa > 0.60 else
    'Moderat (0.41-0.60)'     if cnn_kappa > 0.40 else
    'Cukup (0.21-0.40)'       if cnn_kappa > 0.20 else
    'Lemah (<0.20)'
)
print(f"  → Interpretasi Kappa: {kappa_interp}")

# Ringkasan Hasil SVM (Baseline)
y_pred_svm = svm_pipeline.predict(X_test)
...
summary_svm = pd.DataFrame({
    ...
    'F1-Score' : f1_svm.round(4),
    'Support'  : cm_svm.sum(axis=1)
})

print('\n' + '='*65)
print('RINGKASAN HASIL SVM (Baseline Handcrafted Feature)')
print('='*65)
print(summary_svm.to_string(index=False))
print('='*65)

# Perbandingan Akhir
print('\n--- Perbandingan dengan Baseline ML ---')
print(f'{"Model":<20} {"Accuracy":>10} {"Macro F1":>10} {"Kappa":>10}')
print('-'*52)
print(f'{"SVM":<20} {results["SVM"]["Test"]["acc"]:>10.4f} {results["SVM"]["Test"]["f1"]:>10.4f} {results["SVM"]["Test"]["kappa"]:>10.4f}')
print(f'{"CNN (Ours)":<20} {cnn_acc:>10.4f} {cnn_macro_f1:>10.4f} {cnn_kappa:>10.4f}')

"""Bar Chart Perbandingan CNN vs SVM"""

models     = ['SVM\n(Handcrafted)', 'CNN\n(Transfer Learning)']
accs       = [results['SVM']['Test']['acc'], cnn_acc]
macro_f1s  = [results['SVM']['Test']['f1'],  cnn_macro_f1]
kappas     = [results['SVM']['Test']['kappa'], cnn_kappa]

x   = np.arange(len(models))
w   = 0.25

fig, ax = plt.subplots(figsize=(9, 5))
b1 = ax.bar(x - w,   accs,      w, label='Accuracy',   color='#42A5F5', edgecolor='white')
b2 = ax.bar(x,       macro_f1s, w, label='Macro F1',   color='#66BB6A', edgecolor='white')
b3 = ax.bar(x + w,   kappas,    w, label="Cohen's κ",  color='#FF7043', edgecolor='white')
...
ax.set_ylim(0, 1.15)
ax.set_ylabel('Score')
ax.set_title('Perbandingan Performa: CNN vs SVM (Handcrafted Feature)', fontweight='bold')
ax.legend()
ax.grid(axis='y', alpha=0.3)
plt.tight_layout()
